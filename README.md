...
> Минимальный демо‑запуск возможен без устройств — сгенерируем тестовые данные.
...
Если /dev/ttyUSB0 недоступен, backend автоматически включит генератор тестовых данных.
...
1) Запустите backend (включится генератор данных)
2) Запустите frontend и откройте UI
3) При желании включите CV‑окно отдельно

> ℹ️ Совет: держите открытыми одновременно UI (http://localhost:5173) и страницу теста backend (http://localhost:8080) — будет видно «сырой» поток и визуализацию.

---

## Галерея / Demo

> Замените заглушки на свои скриншоты/гифки из демо.

- ![UI Map Overview](public/demo-map.png)
- ![Sidebar + History](public/demo-sidebar.png)
- ![CV Detection](public/demo-cv.png)
- ![Mobile View](public/demo-mobile.png)

<details>
  <summary>Сценарий живой демонстрации (2–3 минуты)</summary>

1. Открыть UI: показать подключение WebSocket и легенду (WiFi/BLE/CV).
2. Плавно подвигать объект: обратить внимание на круг точности и траекторию.
3. Отключить 1 канал (WiFi или BLE): показать устойчивость (graceful degradation).
4. Включить CV окно: показать detections и глубину (при наличии камеры).
5. Показать калибровку через Serial (одна команда) и эффект на точность.
...
> 💡 Wow‑эффект: радиусы сигналов рисуются полупрозрачными кругами; добавьте лёгкие «пульсации» точек маяков и цели (см. UI). 

## Точность, производительность, масштабирование

- Точность: ±0.5–2.0 м (зависит от условий/калибровки/экранирования)
